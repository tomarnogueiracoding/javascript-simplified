...
function getNoteDetail(keyboardKey) {
  return NOTE_DETAILS.find((el) => `Key${el.key}` === keyboardKey)
}

function playNotes() {
  NOTE_DETAILS.forEach((el) => {
    const keyElement = document.querySelector(`[data-note="${el.note}"]`)
    keyElement.classList.toggle('active', el.active)
    if (el.oscillator != null) {
      el.oscillator.stop()
      el.oscillator.disconnect()
    }
  })

  const activeNotes = NOTE_DETAILS.filter((el) => el.active)
